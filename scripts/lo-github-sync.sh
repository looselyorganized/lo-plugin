...
jobs:
  ...
  # Determine whether the file (if present) was created by this script
  local IS_MANAGED=false
  if [[ -f ".github/workflows/audit.yml" ]] && grep -q 'Auto-generated by lo-github-sync' .github/workflows/audit.yml; then
    IS_MANAGED=true
  fi
  ...
  if [[ "$SHOULD_EXIST" == "true" ]]; then
    if [[ -f ".github/workflows/audit.yml" ]]; then
      if [[ "$IS_MANAGED" == "false" ]]; then
        # Custom file — leave it alone
        if [[ "$FIX" == "true" ]]; then
          ok "audit.yml                   custom, left intact"
        else
          dryrun "audit.yml                   custom, would be changed"
        fi
        return
      fi
    ...
    if [[ -f ".github/workflows/audit.yml" ]]; then
      if [[ "$IS_MANAGED" == "false" ]]; then
        # Custom file — leave it alone regardless of status/language
        ok "audit.yml                   custom, left intact"
        return
      fi
      if [[ "$FIX" == "true" ]]; then
